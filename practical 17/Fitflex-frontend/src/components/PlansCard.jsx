...
function PlansCard({info}) {
  ...
  useEffect(() => {
    ...
    if (info.level === "Beginner") {
      setColor("green");
    } else if (info.level === "Intermediate") {
      setColor("orange");
    } else {
      setColor("red");
    }
  ...
  return (
    ...
    >
      ...
      <CardMedia component="img" height="160" image={info?.imageUrl} alt={info.image} />
      <CardContent >
        <div
          style={{
            position: "absolute",
        ...
        >
          <CardActions sx={{ display: "flex", justifyContent: "space-between"}}>
            <Box sx={{display:"flex", flexDirection:"column", alignItems:"start", justifyContent:"center"}}>
                <Typography variant="h4" sx={{marginBottom:"-10px", lineSpacing:"-3px"}}>{info?.duration}</Typography>
